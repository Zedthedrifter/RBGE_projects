...
def parse_vcf(vcf,output):
  ...
  for i in opener(vcf,'rt'):
    if i.startswith('#CHROM'):
      ...
      if not i.startswith('#') : # and n<=200/ this is a good cap for testing just a small number of SNPs
        n+=1
...
def species_specific_alleles(id_sp,snps,snps_info,high, output):
  ...
  for k,v in snps.items():
    ...
    for al,typ in {0:'REF',1:'ALT'}.items(): #assume two alleles
      ...
      if sum(present)==1 : #the allele is present in only one species
        ssallele[k],target={},[s for s,f in freqs.items() if f >0][0]
...
def specifi_SNPs(vcf,names,high,low,output):
  ...
  snp_out=species_specific_alleles(id_sp,snps,snps_info,int(high),output) #output all species specific snps
