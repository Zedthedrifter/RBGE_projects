...
#git clone https://github.com/smirarab/ASTRAL.git $HOME/projects/rbge/$USER/env/$env_name/bin/ASTRAL
#unzip $HOME/projects/rbge/$USER/env/$env_name/bin/ASTRAL/*zip
cd $HOME/projects/rbge/$USER/env/$env_name/bin/ASTRAL
source $HOME/projects/rbge/$USER/env/$env_name/bin/ASTRAL/make.sh
}
...
mkdir $easy353path
install_easy353
install_astral #also a wee hacking method
install_basic
install_tree
pip_install
...
#this function is reusable, if you make a seperate function for input selection
function run_astral {

INDIR=$1
OUTDIR=$2
OUT_PRE=$3
threshold=$4 #can resolve >= xx phylogeny

echo 'running astral'
#clean off old files of the same name to avoid writing to the same file
rm -f $OUTDIR/${OUT_PRE}.in.treefile
genes=$(cat ${GENE_RESOLUTION}|awk -F','  -v thresh="$threshold" '$8 >= thresh'|cut -f 1 -d ',')
for gene in $genes
do
echo $gene; 
cat $INDIR/${gene}/*treefile >> $OUTDIR/${OUT_PRE}.in.treefile
done
#create astral phylogeny with selected genes
java -jar $astral -i $OUTDIR/${OUT_PRE}.in.treefile -o $OUTDIR/${OUT_PRE}.out.treefile 2>out.log
...
astral=$HOME/projects/rbge/$USER/env/$env_name/bin/ASTRAL/astral.5.7.8.jar #the java script that build an astral tree from a collection of trees
PARENT=$HOME/scratch/$WORKDIR/results
GENE_RESOLUTION=$SCRATCH/Barcoding_km/barcoding_salix_28/results/phy11_easy353_high_resolution_phy/gene_resolution.csv #use the resolution file from the results with higher coverage (47 salix)
...
RESULT20=$SCRATCH/$WORKDIR/results/phy18_astral_trees
...
#./count_mono.py $RESULT12 $RESULT13 #use the result from salix 47 to pick genes
run_astral $RESULT12 $RESULT20 gene353 0
run_astral $RESULT12 $RESULT20 gene353_1 1
run_astral $RESULT12 $RESULT20 gene353_2 2
run_astral $RESULT12 $RESULT20 gene353_3 3
run_astral $RESULT12 $RESULT20 gene353_4 4
}
