...
def rename_fqgz(indir,outdir,prefix):
  ...
  r1=sbp.check_output(f"ls {indir}/*_R1.fastq.gz",shell=True).decode().strip('\n').split('\n')
  r2=sbp.check_output(f"ls {indir}/*_R2.fastq.gz",shell=True).decode().strip('\n').split('\n')
  pairs=[(i,j) for i,j in zip(r1,r2)]
  test=[True if p[0].replace('_R1.fastq.gz','')==p[1].replace('_R2.fastq.gz','') else False for p in pairs ]
  print(test)
  if False not in test: #if all names are well matched
    print('all r1, r2 files are matched. Proceed to renaming')
    rename_dict={p:(f"{prefix}_{i+1}_1.fq.gz",f"{prefix}_{i+1}_2.fq.gz") for i,p in enumerate(pairs)}
    for k,v in rename_dict.items(): #copy the files to new directory, renamed
      sbp.call(f"cp {k[0]} {outdir}/{v[0]}",shell=True)
      sbp.call(f"cp {k[1]} {outdir}/{v[1]}",shell=True)
...
def rename_contig(indir, outdir,infile,fcsv):
  reader = csv.reader(open(f"{fcsv}", 'r'))
  d = {}
  for row in reader:
    k, v1,v2,v3 = row
    d[k] = [v1,v2,v3]
  contigs=SeqIO.to_dict(SeqIO.parse(f"{indir}/{infile}",'fasta'))
  records=[SeqRecord(v.seq,id=d[k][2],name=v.name,description=v.description) for k,v in contigs.items()]
