...
def gene_summary(indir,outdir):
  ...
  names=pd.read_csv(f'{outdir}/sample_names.csv',index_col=0,encoding="latin-1") #avoid UnicodeDecodeError: 'utf-8' codec can't decode byte 0xa0 in position 1607: invalid start byte
...
def gene_select(indir,outdir,cvg):
  ...
  try:
    sbp.call(f"rm {outdir}/rsl_{cvg}_genes.treefile",shell=True)
  except:
